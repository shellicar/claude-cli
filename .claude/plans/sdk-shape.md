...
The SDK was originally designed without any concept of a session, to avoid the kind of persistence the official Claude Code SDK owns (session ids, session files, resume-by-id). Avoiding that persistence was correct. Avoiding any concept of accumulated state at all was wrong. A clean design still needs the concept of a session: state that accumulates across queries and turns for the life of the process. What it does not need is any ability to save or restore that state to or from disk.
...
The correct rule is "no persistence or hydration of a session". Not "no session". The SDK holds session state in memory: the Conversation and the durable config (the usage options the consumer picked once at assembly time). The consumer does not rebuild these on every call; the SDK carries them forward. If the consumer wants to resume across processes, the consumer reads the conversation out, saves it however they want, and on next startup constructs fresh state and pushes the messages back in. The SDK knows nothing about that flow.
...
**Request builder.** Pure function. Given a config and wire-view messages, returns a request body and the request options that accompany it at call time (headers plus per-call concerns such as abort signal and timeout). This is the block that cashes out the "config describes usage, not API shape" principle: usage-level config (compaction, thinking, cache TTL, cached reminders, tool schemas, system prompts) is translated here into the combination of body fields, `cache_control` markers, and `anthropic-beta` header values the API actually requires. If an escape-hatch raw `betas` value is present, it is merged in on top. The system prompt always begins with a fixed SDK identity prefix (`AGENT_SDK_PREFIX`). This is required by the Anthropic API: the SDK cannot make calls without it. It is not a consumer policy decision, not optional, and not overridable. Stateless. Not a class unless there is a reason.

**Tool registry.** Holds tool definitions and executes them. Owns the tool schema in both forms: the Zod schema as the source of truth for validation, and the wire JSON Schema handed to the request builder. Converts Zod to JSON Schema once when a tool enters the registry, not on every request. Validates tool-use input against the Zod schema, calls the handler, formats the result as a `tool_result` block, runs the optional result-transform hook. Knows nothing about approval. Knows nothing about conversations. Just a callable catalog.
...
**Session (state, not a class).** "Session" in this SDK means the state the SDK accumulates across queries for the life of the process. The state is exactly two things: the Conversation (a block, listed above) and the durable config (the usage options the consumer picked once at assembly time: model, tools, betas, system prompts, cache TTL, compaction, approval mode, cached reminders). There is no SDK class called `Session`. There is no `session.query(input)` method. The state is held by whatever constructs the blocks, which in the default assembly (today the CLI) means long-lived fields holding the Client, Conversation, Tool registry, Channel, Approval coordinator, and durable config, plus a call to the SDK's query entry point passing those fields alongside the per-query input (user message, abort controller, system reminder, transform hook). The exact shape of that entry point is the next design decision; whichever shape it takes, it does not construct a Session class. When the process dies, the state dies with it. Persistence and hydration are consumer concerns: the consumer reads the conversation out, saves it however they want, and pushes messages back in on next startup.
