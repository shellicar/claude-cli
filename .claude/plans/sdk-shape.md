...
**Client.** Talks to the Anthropic API. Owns authentication (token acquisition and refresh), HTTP transport, and transport-identifying headers (user-agent, SDK version). It does not decide what goes in a request: it takes a fully-formed body and a fully-formed header set from the request builder and sends them. The feature-level beta headers (compaction, thinking, extended cache TTL, and so on) are chosen by the request builder from the session config, not by the client. The client is the pipe, not the policy. One instance per process. Stateless with respect to conversations. You hand it a token source and it gives you the ability to make authenticated API calls. No other block makes HTTP calls.
