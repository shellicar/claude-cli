...
## Builder — implement the permissions-notice gate (#fix/permissions-print, Phase 3)

Turned the Phase 2 scaffold green. `PermissionsNoticeGate.update` now gates the 🔔 notice on a *displayed-permissions* change, and the stale-named `formatPermissionChange.ts` is renamed to match its contents. All 800 tests pass; type-check clean.

### What I implemented

- **`PermissionsNoticeGate.update(permissions)`** (`src/model/PermissionsNoticeGate.ts`). Renders `formatPermissionsDisplay(permissions)`, compares it to `#lastDisplay`, returns `null` when identical, otherwise advances `#lastDisplay` and returns the new render to splice. The gate is on the *rendered string*, exactly the SC's decided direction (investigation Option C): the notice never fires when nothing visibly changed, always fires when it does. The constructor's baseline seed (from Phase 2) means startup is silent and a `model`-only edit produces an identical render → `null`.
- **Rename** `src/cli-config/formatPermissionChange.ts` → `formatPermissionsDisplay.ts` via `git mv` (pure rename, 0 content lines changed — git detected 100% similarity). The file holds only `formatPermissionsDisplay` + `PermissionsConfigInput`, so the name now matches.

### Importers updated for the rename

Only two referenced the old path, both updated to `formatPermissionsDisplay.js`:
- `src/model/PermissionsNoticeGate.ts` (source import)
- `test/PermissionsNoticeGate.spec.ts` (type import)

`main.ts` did **not** need touching — Phase 2 had already dropped its now-dead `formatPermissionsDisplay` import, so it never referenced the renamed file. I searched the whole `apps/claude-sdk-cli` tree for `formatPermissionChange`; those two were the only hits.

### Test change (documented per the Builder agent)

The only test edit was the import path in `PermissionsNoticeGate.spec.ts`, a mechanical consequence of the rename — not a change to behaviour, fixtures, or assertions. The three rules passed on the implementation without any change to what they test. Had I left the path stale the spec would not compile.

### Verify results

- `pnpm exec vitest run` (cli pkg): **800 passed** (48 files). The two gate tests that failed `not implemented` against the stub are now green; rule 1 still passes; no existing test broke.
- `pnpm exec tsc --noEmit -p tsconfig.check.json`: clean (exit 0). Ran `tsc` directly rather than `turbo run type-check`, which cache-hits and would not have re-checked the changed files — same reason the Scaffolder gave.

### Things the next operator (Courier) should know

- **The two biome warnings the Scaffolder flagged are gone.** `update` now reads `#lastDisplay` and uses the `permissions` param, so `noUnusedPrivateClassMembers` and `noUnusedFunctionParameters` no longer fire. I did not run `biome`; this is by inspection of what those warnings were about. Ship phase owns formatting.
- **Staged, not committed.** Staged set is exactly: the rename, `PermissionsNoticeGate.ts`, `PermissionsNoticeGate.spec.ts`. `node_modules` was already installed by Phase 2; nothing tracked beyond these three moved.
- **No scope creep.** I built only what the tests require and renamed the one file the brief named. No new abstractions, no extra fields on the gate.

### Proposed commit message

`Show permissions notice only when displayed permissions change`
