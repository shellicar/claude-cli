...
## Courier — ship: permissions notice gates on displayed permissions (fix/permissions-print)

The 🔔 permissions notice now prints only when the *displayed* permissions change, not on every `sdk-config.json` edit. Shipped as a PR against main. This entry is the curated arc for the next operator; the dated phase records remain in `2026-06-13.md` (investigation) and `2026-06-14.md` (scaffold).

### What shipped and how it works

- **`src/model/PermissionsNoticeGate.ts`** — `update(permissions): string | null` renders `formatPermissionsDisplay(permissions)`, compares it to `#lastDisplay`, returns `null` when identical, else advances `#lastDisplay` and returns the new render to splice. The gate is on the *rendered matrix string*, not a field-by-field compare, so a `model`-only edit yields an identical render and prints nothing.
- Baseline `#lastDisplay` is seeded at construction from the already-loaded config, so startup is silent.
- Registered in `container.ts` next to `ApprovalNotifier` (both `ConfigLoader`-derived); `entry/main.ts` onChange splices only when `update` returns non-null. The old unconditional `spliceNotice(...)` at `main.ts:214` is gone.
- Stale `formatPermissionChange.ts` renamed to `formatPermissionsDisplay.ts` (it only held that function).

### Why it broke (the trap)

The gate existed for ~84s on the feature branch (`2c7ea94` added it, `8e529f6` removed it as "redundant") and was squashed into `1676610`, so it never reached main. The regression was invisible because the only test covered a pure function that became dead code and was deleted; nothing pinned config-change to notice. The new spec drives the real render path (`update` -> `spliceNotice` -> `renderConversation`), which is the seam that was missing. Keep that seam: a pure-function test would rot the same way.

### Traps for the next operator

- **Testament union-merge.** This branch's `2026-06-14.md` and `2026-06-15.md` share filenames with main's own dated testaments (from #351/#352). The `.claude/testament/.gitattributes` union driver concatenates both sides on merge, so expect both sets of content there post-merge. That is by design, not a conflict.
- **`--follow` lies here.** The behaviour-changing commits do not show in `--follow` on the formatter file, because the behaviour lived in `main.ts`, not the formatter.

### Ship

- `changes.jsonl` (claude-sdk-cli): one `fixed` entry for the effect.
- main advanced to `e6b9e4c` during the work (#351 Fable 5 cost/context, #352 esbuild override). Clean merge, no overlap with our files; pushed without pre-merging.
