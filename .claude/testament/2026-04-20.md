...
## Append operation for EditFile (#259)

### Where it lives

`append` is a parameter on `PreviewEditInputSchema`, not `EditFileInputSchema`. The mission says "EditFile input schema" but `lineEdits` and `textEdits` (the mutually exclusive counterparts) only exist on `PreviewEdit`. The append goes through the same diff/stage/apply flow as all other edits.

### Schema refine

`PreviewEditInputSchema` has a `.refine()` that validates at least one of `lineEdits`, `textEdits` is non-empty. This needed updating to also allow `append != null`. Without it, `call(previewEdit, { file, append: '...' })` throws a Zod validation error before the handler runs.

### Mutual exclusion errors

Tests 4 and 5 use `.rejects.toThrow('append')`. The error messages are `'append is mutually exclusive with lineEdits'` and `'append is mutually exclusive with textEdits'`. Both contain the word 'append', which is all the matcher needs.

### ENOENT

Test 6 (append to nonexistent file) passes without any explicit error handling. The append branch calls `fs.readFile(filePath)` before writing, which throws ENOENT naturally from `MemoryFileSystem` when the file doesn't exist.

### Concatenation

`newContent = baseContent + input.append`. No special trailing-newline handling. Correct for all cases.

### Test file
# --file flag (#278)

## parseArgs type widening trap

`parseArgs` with `strict: false` widens all option values to `string | boolean`. Testing `!= null` is not a sufficient type guard for calling `.replace()` on a string. Use `typeof values.file === 'string'` at the point where you resolve the path.

## ConversationSession: startFresh() vs createNew() vs load()

Three startup paths exist and they are not interchangeable:

- `load()` — reads the existing `.claude/.sdk-conversation-id` marker and resumes that session
- `createNew()` — saves the current conversation, then resets to a fresh UUID (for the `n` key flow)
- `startFresh()` — generates a new UUID and writes the marker, skipping all history; safe to call at startup before any conversation exists

`--file` uses `startFresh()`. Using `createNew()` would try to save a conversation that doesn't exist yet.

## Skipping waitForInput() on the first iteration

`AppLayout` starts in `'editor'` mode. `startStreaming()` (called by `runAgent`) transitions unconditionally. After `completeStreaming()` returns, the layout is back in a clean state and the next `waitForInput()` call works normally. Skipping it for the first iteration when `--file` is provided is safe.

The `buildSubmitText` + inline `FileAttachment` path mirrors exactly what the `f` key does in command mode, just without the `CommandModeState` state machine.

## IFileSystem.stat() lacks isDirectory

`StatResult = { size: number }` only. Two places in `apps/claude-sdk-cli/src/` import `stat` directly from `node:fs/promises` because `nodeFs.stat()` can't tell them whether the path is a directory:

- `AppLayout.ts` (the `f` key handler)
- `entry/main.ts` (`buildFileInput` for `--file`)

The fix is to extend `StatResult` with `isDirectory: boolean` and update `NodeFileSystem.stat()`. Known debt, not addressed here.

## Pre-existing CI failure

`packages/claude-core/src/reflow.ts:123` — biome reports `noControlCharactersInRegex`. Present before this work. CI will flag it but it is not this PR's failure.

## Biome ternary formatting

Multi-line ternaries of the form `const x =\n  condition ? a : b` trigger a formatting error if the expression fits on one line. `pnpm ci:fix` will reformat them. Run it twice if the first pass produces a diff; it stabilises on the second run.

## mcp-exec test PATH trap
...
`packages/claude-sdk-tools/test/EditFile-append.spec.ts`. One `expect` per `it`, `expected`/`actual` variables throughout.
`test/cli.spec.ts` in `packages/mcp-exec/` spawns `tsx` by name via `StdioClientTransport`. Under `pnpm test` this works because `node_modules/.bin` is on PATH. Under the VS Code vitest extension it fails because that PATH is not inherited. Fix: resolve `tsx` to an absolute path via `path.join(packageRoot, '../../node_modules/.bin/tsx')`. Pre-existing issue, not related to this PR.

# 10:40

## Release 1.0.0-beta.4

**Branch:** `feature/version-beta4`, created from `origin/main`.

**All four packages bumped** — `claude-sdk-cli`, `claude-sdk-tools`, `claude-sdk`, `claude-core` all had changes in `src/` or `changes.jsonl` since the `claude-sdk-cli@1.0.0-beta.3` tag.

**GitVersion.yml fix:** Replaced `Mainline` strategy with `TaggedCommit` + `Fallback`, and changed `label` from `alpha` to `beta`. Also removed the `increment: Patch` override under `branches.main` (it was redundant with the top-level setting). The old `Mainline` strategy was causing version calculation issues.

**claude-core note:** The `reflow.ts` change since beta.3 (adding `biome-ignore lint/suspicious/noControlCharactersInRegex` comment) was NOT documented in `changes.jsonl`. The existing entry "Package now publishes CJS alongside ESM with working sourcemaps" was already present before beta.3 but had no release marker, so it rolled up into beta.4. The `reflow.ts` biome-ignore fix is the pattern established earlier today — see the prior testament entries about `ci:fix and reflow.ts`.

**Staged files (after testament):**
- `.claude/CLAUDE.md` and `.claude/testament/2026-04-20.md` (already staged from preflight)
- `GitVersion.yml`
- `apps/claude-sdk-cli/{package.json,changes.jsonl,CHANGELOG.md}`
- `packages/claude-sdk/{package.json,changes.jsonl,CHANGELOG.md}`
- `packages/claude-sdk-tools/{package.json,changes.jsonl,CHANGELOG.md}`
- `packages/claude-core/{package.json,changes.jsonl,CHANGELOG.md}`

No `pnpm-lock.yaml` changes (version bumps do not affect the lockfile).
