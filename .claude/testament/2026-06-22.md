...
# 08:32 — beta.10 release fired, publish FAILED on OIDC (Phase 3)

**Where it stands: STOPPED, surfaced, not retried.** The GitHub release `claude-sdk-cli@1.0.0-beta.10` exists (created against merged-main SHA `db019d9e`, `--prerelease`, notes from `[Unreleased]`). But the `npm-publish.yml` run (`27919520607`) **failed**, and **nothing reached npm** — neither the platform package nor the launcher. The release tag is live; the packages are not.

**Root cause — OIDC trusted publishing is not configured for the brand-new platform package.** The `platform` job (`macos-14`/`darwin-arm64`) died at `publish-package`:
```
[WARN] Skipped OIDC: ERR_PNPM_AUTH_TOKEN_EXCHANGE: Failed token exchange request ... 404
[E404] 404 Not Found - PUT https://registry.npmjs.org/@shellicar%2fclaude-sdk-cli-darwin-arm64 - Not found
```
The OIDC token exchange with npm returned 404 (no trust configured), so pnpm skipped OIDC and fell through; the PUT to create the brand-new scoped package then 404'd. Confirmed `@shellicar/claude-sdk-cli-darwin-arm64` **does not exist on npm at all** (`npm view` → E404) — this is its first-ever publish. The launcher `main` job was **skipped** (`needs: [prepare, platform]`), so the launcher never even attempted; it remains at `beta` → `1.0.0-beta.9`.

**Why this is the OIDC-trust case, not a token to rotate.** The skill is explicit: a publish-auth failure under trusted publishing means the trust isn't configured on npmjs.com for this package+workflow — it is *not* an expired token, so don't go hunting for one. For a brand-new package the trust can't have been pre-configured against a package that doesn't exist yet. This is the SC's action: establish trusted publishing for `@shellicar/claude-sdk-cli-darwin-arm64` (and verify it for the launcher) on npmjs.com, or seed the package first. This needs npm account access I don't have.

**Mission said stop on publish failure — I did.** "If the publish workflow fails ... stop and surface it — don't rerun blindly past a real failure." A rerun changes nothing until the npm-side trust exists. The acceptance check (`npm i -g …@beta` launching the full CLI) was not run — there is nothing published to install.

**For whoever picks this up:** the GitHub release already exists. Once the npm trust is fixed, the clean path is `gh run rerun 27919520607 --repo shellicar/claude-cli` (re-fires the same publish against the same tag) rather than deleting/recreating the release. Verify both `@shellicar/claude-sdk-cli-darwin-arm64@1.0.0-beta.10` and the launcher land, then run the install-and-launch acceptance.
