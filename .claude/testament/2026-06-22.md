...
# 06:44 — Phase 1 Maker: beta.10 backfill + bump

Cutting `1.0.0-beta.10`. Two findings worth carrying:

**The mission's premise was stale.** It said the #363 Courier added no `changes.jsonl` entries, so the SEA work had no changelog record. Not true: `git show 1c7e918 -- apps/claude-sdk-cli/changes.jsonl` shows #363 added exactly one entry — the SEA `changed` line ("Ship the CLI as a prebuilt Single Executable Application... node:sqlite store runs on the bundled Node 26..."). That one line already covers the scope the mission named as the backfill target. I stopped and surfaced this rather than inventing duplicate entries. The SC chose option B: keep the existing `changed` entry, add one complementary `fixed` entry for the *resolved crash* (the existing line is framed as a capability change; it never names the `NODE_MODULE_VERSION`/better-sqlite3 bindings crash a user actually hit). That's the only entry I added.

**pnpm version refuses on a dirty tree.** Because I appended the changes.jsonl entry first, `pnpm version 1.0.0-beta.10 --no-git-tag-version` failed with `ERR_PNPM_UNCLEAN_WORKING_TREE`. Add `--no-git-checks` to bypass the clean-tree guard. (Alternative: bump before editing changes.jsonl.)

**Mechanics that worked:** changelog regen is `pnpm exec tsx src/generate-changelog.ts ../apps/claude-sdk-cli` from `scripts/` (no `changelog` npm alias exists, only `validate`/`schema`). Validate with `pnpm exec tsx src/validate-changes.ts` — "no errors found". `pnpm install` reported "Already up to date": version bumps don't touch the lockfile because workspace deps resolve via `workspace:*`, so there's no `pnpm-lock.yaml` to stage. Pre-release stays under `[Unreleased]`, no version heading — confirmed in the regenerated CHANGELOG.

Staged: `apps/claude-sdk-cli/{changes.jsonl,CHANGELOG.md,package.json}` and `platforms/claude-sdk-cli-darwin-arm64/package.json`. No commit — left for the SC.
