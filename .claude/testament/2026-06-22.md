...
# 06:55 — Apostle, messages-api-layer Phase 2 iteration 4

(Different mission arc from the SQLite/SEA legacy above — this is the `@anthropic-ai/sdk` runtime-replacement plan. Co-existing in today's shared testament.)

## The direction (from the mission Delivery Notes, 2026-06-22)

Iteration 3 tested `parseSse` directly but left `parseFrame`/`findFrameBoundary` as internal helpers, covered only through it. The SC's standard: foundational API-communication code must work perfectly, so pin it at the unit level — both **exported** and **tested directly and exhaustively**. Option B and the rest of the plan stand.

## What I changed (surgical patch, not overwrite)

- §2 `sse.ts`: exported `parseFrame`, `findFrameBoundary`, and the `SseFrame` type; one prose sentence on why. `handleFrame` stays internal — the SC named only the two framing primitives; `handleFrame` (ping/error/JSON.parse) is already covered by the `parseSse` direct group.
- §9: two new exhaustive direct test groups — `findFrameBoundary` (LF/CRLF earliest-wins selection, neither→null, index 0) and `parseFrame` (event+data, data-only, multi-data join, leading-space strip both ways, comment ignore, trailing-`\r` strip, no-colon line, empty→null, event-without-data→`{event, data:''}`).
- Method: 4 textEdits (+23/−4), no behavioural change to either function — visibility only.

## Co-working state (worktree moved under me)

Target HEAD advanced externally this cast (`c1a9b62`→`1c7e918`: single-executable packaging etc. — the arc the testament above documents). My `2026-06-21.md` is still staged. `2026-06-16.md` shows modified-unstaged — not mine, left untouched. I staged only my plan (fleet repo) and the testament; explicit paths, no `git add .`.

## For the Maker

`parseFrame`/`findFrameBoundary` exported from `sse.ts` — unit-test directly per §9. Plan otherwise as iteration 3 (Option B: raw-event async iterable, accumulation in `StreamProcessor`, give-up via `AccountLimitStoppedError`).
