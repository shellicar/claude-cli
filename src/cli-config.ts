...
const cliConfigSchema = z
  .object({
    ...
    extendedPermissionTimeoutMs: z.int().min(1000).nullable().optional().default(120_000).catch(120_000).describe('Timeout in milliseconds for extended permission prompts (e.g. EnterPlanMode, ExitPlanMode). Set to null to disable timeout.'),
    questionTimeoutMs: z.int().min(1000).nullable().optional().default(60_000).catch(60_000).describe('Timeout in milliseconds for user question prompts. Set to null to disable timeout.'),
