...
describe('createReadFile \u2014 success', () => {
  it('returns lines as content output', async () => {
    ...
    const result = await ReadFile.handler({ path: '/src/hello.ts' }, new Map());
  ...
  it('returns a single-element array for a single-line file', async () => {
    const ReadFile = createReadFile(makeFs());
    const result = await ReadFile.handler({ path: '/src/single.ts' }, new Map());
    expect(result).toMatchObject({
      type: 'content',
      values: ['single line'],
      totalLines: 1,
  ...
  it('returns correct totalLines matching values length', async () => {
    const ReadFile = createReadFile(makeFs());
    const result = await ReadFile.handler({ path: '/src/hello.ts' }, new Map());
    const content = result as { type: 'content'; values: string[]; totalLines: number };
  ...
  it('echoes the resolved path in the output', async () => {
    const ReadFile = createReadFile(makeFs());
    const result = await ReadFile.handler({ path: '/src/hello.ts' }, new Map());
    const content = result as { path: string };
    expect(content.path).toBe('/src/hello.ts');
  });
});
...
describe('createReadFile \u2014 error handling', () => {
  it('returns an error object for a missing file', async () => {
    const ReadFile = createReadFile(makeFs());
    const result = await ReadFile.handler({ path: '/src/missing.ts' }, new Map());
    expect(result).toMatchObject({
      error: true,
