...
function makeDeps(executor: FakeExecutor) {
  return { executor, getCert: () => 'cert', getClientId: () => 'client-id', getTenantId: () => 'tenant-id' };
}

describe('AzureDevOps V2', () => {
  it('every tool is registered as escalate — always gated, never a pre-trustable fs.* tier', () => {
    const executor = new FakeExecutor();
    const tools = createAdoPrToolsV2(makeDeps(executor), () => ({ acct: { tenantId: 't', readerClientId: null, holderClientId: 'c' } }), new AzSessionCache(Clock.systemUTC()));
  ...
  it('runs the Ready tool against the sole configured holder account, in a directory with no git remote', async () => {
    ...
    const [Create, Ready] = createAdoPrToolsV2(makeDeps(executor), () => ({ acct: { tenantId: 't', readerClientId: null, holderClientId: 'c' } }), cache);
    void Create;
