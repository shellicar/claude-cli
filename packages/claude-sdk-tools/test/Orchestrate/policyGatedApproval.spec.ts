...
describe('createPolicyGatedApproval \u2014 an allow verdict', () => {
  it('approves the call', async () => {
    ...
    const expected = true;
    const actual = (await approve({ name: 'Program', operation: 'fs.exec', input: {}, asWritten: {}, batch: [], stagePosition: 1, stageCount: 1 })).approved;
  ...
  it('never asks a human', async () => {
    ...
    await approve({ name: 'Program', operation: 'fs.exec', input: {}, asWritten: {}, batch: [], stagePosition: 1, stageCount: 1 });
...
describe('createPolicyGatedApproval \u2014 a deny verdict', () => {
  ...
  it('carries the policy message through', async () => {
    ...
    const outcome = await approve({ name: 'Program', operation: 'fs.exec', input: {}, asWritten: {}, batch: [], stagePosition: 1, stageCount: 1 });
...
describe('createPolicyGatedApproval \u2014 path extraction', () => {
  it('extracts the tool\u2019s own marked path field so a path-scoped rule can actually match', async () => {
    ...
    const expected = false;
    const actual = (await approve({ name: 'Find', operation: 'fs.list', input: { path: '/inside/dir' }, asWritten: { path: '/inside/dir' }, batch: [], stagePosition: 1, stageCount: 1 })).approved;
    expect(actual).toBe(expected);
  });
  ...
  it('a path-scoped rule does not match when the tool\u2019s path is outside the rule\u2019s pattern', async () => {
    ...
    const expected = true;
    const actual = (await approve({ name: 'Find', operation: 'fs.list', input: { path: '/outside/dir' }, asWritten: { path: '/outside/dir' }, batch: [], stagePosition: 1, stageCount: 1 })).approved;
    expect(actual).toBe(expected);
  });
  ...
  it('a tool with no registered schema extracts no paths, so a real (non-wildcard) path-scoped rule cannot match it', async () => {
    ...
    const expected = true;
    const actual = (await approve({ name: 'UnknownTool', operation: 'fs.exec', input: { path: '/anything' }, asWritten: { path: '/anything' }, batch: [], stagePosition: 1, stageCount: 1 })).approved;
    expect(actual).toBe(expected);
  });
});
