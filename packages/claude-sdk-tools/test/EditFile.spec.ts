...
describe('createEditFile \u2014 staging', () => {
  it('stores a patch in the store and returns a patchId', async () => {
    ...
    const result = await EditFile.handler(
      { file: '/file.ts', edits: [{ action: 'insert', after_line: 0, content: '// header' }] },
      store,
    );
  ...
  it('computes the correct originalHash', async () => {
    ...
    const EditFile = createEditFile(fs);
    const result = await EditFile.handler(
      { file: '/file.ts', edits: [{ action: 'delete', startLine: 1, endLine: 1 }] },
      new Map(),
    );
  ...
  it('includes a unified diff', async () => {
    ...
    const EditFile = createEditFile(fs);
    const result = await EditFile.handler(
      { file: '/file.ts', edits: [{ action: 'replace', startLine: 2, endLine: 2, content: 'line TWO' }] },
      new Map(),
    );
...
describe('createConfirmEditFile \u2014 applying', () => {
  it('applies the patch and writes the new content', async () => {
    ...
    const staged = await EditFile.handler(
      { file: '/file.ts', edits: [{ action: 'replace', startLine: 1, endLine: 1, content: 'line ONE' }] },
      store,
    );
    const confirmed = await ConfirmEditFile.handler({ patchId: staged.patchId }, store);
  ...
  it('throws when the file was modified after staging', async () => {
    ...
    // Mutate the file after staging
    await fs.writeFile('/file.ts', 'completely different content');

    await expect(ConfirmEditFile.handler({ patchId: staged.patchId }, store)).rejects.toThrow(
  ...
  it('throws when patchId is unknown', async () => {
    ...
    const ConfirmEditFile = createConfirmEditFile(fs);
    await expect(
      ConfirmEditFile.handler({ patchId: '00000000-0000-4000-8000-000000000000' }, new Map()),
