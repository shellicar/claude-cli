...
import { resolveSince } from './resolveSince';
 ...
 * conversation); it is what `includeCurrentSession: false` holds out of results. `now` sources the clock a
 * relative `since` span is measured back from.
 */
export function createHistoryTools(reader: IHistoryReader, currentSessionId: () => string, now: () => Date = () => new Date()) {
  const SearchHistory = defineTool({
    name: 'SearchHistory',
    operation: 'read',
    description: 'Search your past conversations by relevance and get back ranked, cited snippets. A citation is a session id plus a turn number; pass one (or several) to ReadHistory to open the full exchange around it. Thinking is indexed and ranks on par with prose — the reasoning in a thinking block is often the most descriptive account of what a piece of work was.',
    ...
    handler: async (input) => {
      const since = input.since === undefined ? undefined : resolveSince(input.since, now());
      const excludeConversationId = input.includeCurrentSession ? undefined : currentSessionId();
      const hits = reader.search({ query: input.query, role: input.role, type: input.type, since, excludeConversationId, limit: input.limit });
      const out = hits.map((hit) => ({ session: hit.conversationId, turn: hit.turn, timestamp: hit.timestamp, role: hit.role, type: hit.type as EventType, snippet: hit.snippet }));
  ...
  const ReadHistory = defineTool({
    name: 'ReadHistory',
    operation: 'read',
    description: 'Open the full exchange around one or more search citations. Each citation is a { session, turn } from a SearchHistory hit; the shared `window` sets how many turns either side of each centre to include. Each event text is capped so one giant tool_result cannot flood context.',
    input_schema: ReadHistoryInputSchema,
    output_schema: ReadHistoryOutputSchema,
    input_examples: [{ citations: [{ session: 'a1b2c3', turn: 42 }] }, { citations: [{ session: 'a1b2c3', turn: 42 }, { session: 'd4e5f6', turn: 7 }], window: 5 }],
    handler: async (input) => {
      const citations = input.citations.map((citation) => ({ conversationId: citation.session, turn: citation.turn }));
      const windows = reader.read({ citations, window: input.window });
      const out = windows.map((window) => ({
        session: window.conversationId,
        turn: window.turn,
        events: window.events.map((event) => ({ turn: event.turn, timestamp: event.timestamp, role: event.role, type: event.type as EventType, text: event.text })),
      }));
