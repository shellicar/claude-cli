...
- ExecV3's safety rules are now config-driven, not fixed: tools.rules can override, remove, or add to the built-in rules, read live so a config change takes effect immediately with no restart
...
- A rule with no matcher fields is rejected instead of silently matching every command
