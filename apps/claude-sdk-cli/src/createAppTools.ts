...
import { type BlockedCommand, configureExecV3, type IEnvProvider, type RuleOverrideMap } from '@shellicar/claude-sdk-tools/ExecV3';
...
export type CreateAppToolsOptions = {
  ...
  toolsConfig: { exec: boolean; execV2: boolean; execV3: boolean; blockedCommands?: BlockedCommand[]; rules?: RuleOverrideMap };
...
export function createAppTools({ fs, tsServer, toolsConfig, objects, memory, history, currentSessionId, clock, tsAvailable, logger, skillDirs = [], secrets, envProvider, azAccounts }: CreateAppToolsOptions): AppTools {
  ...
  if (toolsConfig.execV3) {
    tools.push(configureExecV3(envProvider, toolsConfig.blockedCommands ?? [], toolsConfig.rules ?? {}));
  }
  tools.push(Ref);
