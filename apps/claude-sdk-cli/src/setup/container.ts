...
export function buildContainer(options: ContainerOptions): IServiceProvider {
  ...
  services.register(ConversationSession).to(ConversationSession, (x) =>
    new ConversationSession(nodeFs, x.resolve(Conversation)),
  );
  ...
  services.register(ModelOverrides).to(ModelOverrides, (x) =>
    new ModelOverrides(modelOverride, x.resolve(StatusState)),
  );
  ...
  services.register(AppToolsService).to(AppToolsService, (x) =>
    new AppToolsService(x.resolve(TsServerService), x.resolve(ConfigLoader)),
  );

  // --- audit ---
  services.register(AuditWriter).to(AuditWriter, () =>
    new AuditWriter(nodeFs, `${nodeFs.homedir()}/.claude/audit`),
  );
  ...
  services.register(ApprovalNotifier).to(ApprovalNotifier, (x) =>
    new ApprovalNotifier(x.resolve(ConfigLoader), x.resolve(NodeProcessLauncher)),
  );

  // --- handlers ---
  services.register(CommandIntentExecutor).to(CommandIntentExecutor, (x) =>
    new CommandIntentExecutor(
      x.resolve(CommandModeState),
      x.resolve(ConversationState),
      x.resolve(ConversationSession),
      x.resolve(NodeAttachmentSource),
      x.resolve(ModelOverrides),
    ),
  );
  services.register(QuitHandler).to(QuitHandler, (x) =>
    new QuitHandler(() => x.resolve(TerminalRenderer).exit()),
  );
  services.register(ApprovalHandler).to(ApprovalHandler, (x) =>
    new ApprovalHandler(x.resolve(ToolApprovalState)),
  );
  services.register(CommandKeyHandler).to(CommandKeyHandler, (x) =>
    new CommandKeyHandler(x.resolve(CommandModeState), COMMAND_BINDINGS_BY_CONTEXT, x.resolve(CommandIntentExecutor)),
  );
  services.register(CancelHandler).to(CancelHandler, (x) =>
    new CancelHandler(() => x.resolve(ConsumerChannel).send({ type: 'cancel' })),
  );
  services.register(EditorHandler).to(EditorHandler, (x) =>
    new EditorHandler(x.resolve(EditorState), x.resolve(CommandModeState), x.resolve(TerminalState)),
  );

  // --- rendering ---
  services.register(TerminalRenderer).to(TerminalRenderer, (x) =>
    new TerminalRenderer(new StdoutScreen(), x.resolve(TerminalState)),
  );
  services.register(PrimaryPresentation).to(PrimaryPresentation, (x) => {
    const editorChain: readonly InputHandler[] = [
      x.resolve(QuitHandler),
      x.resolve(ApprovalHandler),
      x.resolve(CommandKeyHandler),
      x.resolve(EditorHandler),
    ];
    const streamingChain: readonly InputHandler[] = [
      x.resolve(QuitHandler),
      x.resolve(ApprovalHandler),
      x.resolve(CancelHandler),
    ];
  ...
  services.register(ViewHost).to(ViewHost, (x) => {
    ...
    const presentations: ReadonlyMap<AppModeKey, Presentation> = new Map([
  ...
  services.register(TerminalInput).to(TerminalInput, (x) =>
    new TerminalInput(x.resolve(ViewHost)),
  );
  services.register(ReadLine).to(ReadLine, (x) =>
    new ReadLine((key) => x.resolve(TerminalInput).handle(key)),
  );

  // --- durable config (replaces mapConfig() closure) ---
  services.register(DurableConfigFactory).to(DurableConfigFactory, (x) =>
    new DurableConfigFactory(x.resolve(ConfigLoader), x.resolve(ModelOverrides), x.resolve(AppToolsService)),
  );

  // --- query pipeline ---
  services.register(ToolRegistry).to(ToolRegistry, (x) =>
    new ToolRegistry(x.resolve(AppToolsService).tools, logger),
  );
  services.register(TurnRunner).to(TurnRunner, (x) =>
    new TurnRunner(x.resolve(AnthropicClient), x.resolve(StreamProcessor), logger),
  );
  services.register(QueryRunner).to(QueryRunner, (x) =>
    new QueryRunner(
      x.resolve(TurnRunner),
      x.resolve(Conversation),
      x.resolve(ToolRegistry),
      x.resolve(ApprovalCoordinator),
      x.resolve(SdkChannel),
      x.resolve(DurableConfigFactory).config,
      logger,
    ),
  );
