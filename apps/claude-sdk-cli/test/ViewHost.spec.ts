...
describe('ViewHost — escape routing through the primary chains', () => {
  function setup() {
    ...
    services.register(TurnClock).as(ITurnClock);
    services.register(CommandModeState).using(() => model.commandModeState).asSelf();
    services.register(ConversationState).using(() => model.conversationState).asSelf();
    services.register(ConversationSession).using(() => model.session).asSelf();
    // ConversationSession's own @dependsOn(SqliteSessionStore) is declared statically, so v5's engine
    ...
    services.register(ToolApprovalState).using(() => model.toolApprovalState).asSelf();
    services.register(EditorState).using(() => model.editorState).asSelf();
    services.register(TerminalState).using(() => model.terminalState).asSelf();
    services.register(Conversation).using(() => new Conversation()).asSelf();
