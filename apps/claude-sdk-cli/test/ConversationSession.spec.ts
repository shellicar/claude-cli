...
describe('ConversationSession — saveConversation', () => {
  ...
  it('restores a conversation whose tail is an unanswered assistant tool_use verbatim, without healing it', async () => {
    const id = 'a1b2c3d4-e5f6-7890-abcd-ef1234567890';
    const historyPath = `${HOME}/.claude/conversations/${id}.jsonl`;
    ...
    // load restores history as-is; healing is TurnRunner's job at request-build time, not load's.
    const expected = 2;
    const actual = conversation.messages.length;
    expect(actual).toBe(expected);
  });
