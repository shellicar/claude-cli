...
const COLS = 80;
const drawn = (): string[] => box(['const a = 1;'], 'typescript', COLS, true).lines;
const bare = (): string[] => box(['const a = 1;'], 'typescript', COLS, false).lines;
...
describe('box — drawn without an affordance', () => {
  it('shows no copy icon', () => {
    const expected = false;
    const actual = bare()[0]?.includes(COPY_ICON);
    expect(actual).toBe(expected);
  });

  it('reports no icon column', () => {
    const expected = -1;
    const actual = box(['const a = 1;'], 'typescript', COLS, false).iconCol;
    expect(actual).toBe(expected);
  });
