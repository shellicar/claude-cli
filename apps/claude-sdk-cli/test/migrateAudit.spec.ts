...
describe('commit — safety self-check', () => {
  it('does not swap when the assistant-line count is not preserved', async () => {
    ...
    const summary = { scanned: 0, migrated: 0, unchanged: 0, skipped: 0, failed: 0, pairing: { exact: 0, inferred: 0, unpaired: 0 } };
    await commit(fs, A, `${A}/${S}.jsonl`, original, auditLines, broken, { exact: 0, inferred: 0, unpaired: 0 }, S, summary);
  ...
  it('aligns then commits a real insert through the same boundary', async () => {
    ...
    const summary = { scanned: 0, migrated: 0, unchanged: 0, skipped: 0, failed: 0, pairing: { exact: 0, inferred: 0, unpaired: 0 } };
    await commit(fs, A, `${A}/${S}.jsonl`, original, auditLines, output, counts, S, summary);
