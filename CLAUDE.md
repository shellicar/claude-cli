...
This rule is about resolved npm dependencies — a `dependencies`/`devDependencies` entry a consumer's package manager fetches at its own pinned version. A *bundled* dependency (inlined into the output at build time, per the Bundling section above) is different: its code is already baked into the bundling package's `dist`, so there is no version pin for a bump to re-pin. If the bundled source changed, the bundling package's own `dist` is stale regardless of any version field, and it must be bumped and rebuilt to ship that change to its consumers — check this by hand for any package that bundles another; it is not caught by "has this package's own `src/` changed".
