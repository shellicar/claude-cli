...
This rule is about resolved npm dependencies — a `dependencies`/`devDependencies` entry a consumer's package manager fetches at its own pinned version. A *bundled* dependency (inlined into the output at build time, per the Bundling section above) is different: its code is already baked into the bundling package's `dist`, so there is no version pin for a bump to re-pin. The bundling package needs a bump only when the bundled *behavior* it actually imports changed — not merely when a file somewhere in the dependency touched. An internal rename, moved file, or reflowed comment produces identical compiled output once inlined, so it's not a trigger; read the diff on the specific exports the bundling package imports, not just its file list. Check this by hand for any package that bundles another; it is not caught by "has this package's own `src/` changed".
...
**No real spawn, no real disk, outside `test/integration/`.** A test that exercises Exec/ExecV2/ExecV3 must use `FakeExecutor` (`packages/claude-sdk-tools/test/FakeExecutor.ts`), never the real `Executor`/`executor`/`nodeFs` — a rule-matching bug in a test wired to the real executor doesn't fail loud, it runs the real command for real (this happened once: a test asserting `find . -exec rm {} \;` was blocked instead deleted the package, because the block had a bug and the executor was real). Sqlite tests use `new DatabaseSync(':memory:')` unless the behaviour under test specifically requires a second real connection to the same file (WAL/lock contention — an in-memory db can't be shared across connections). A test needing real fs reads (e.g. loading a checked-in fixture) must keep every path contained under `test/` — never `../` out of it — so containment is a directory boundary, not something to trust a path string on.

`test/integration/` is the one exception, for cases where a real OS resource (a real spawned process, a real tsserver, a second real sqlite connection) is the only way to test the thing at all. Each package has its own `vitest.integration.config.ts`, included only via `pnpm test:integration` (also run in CI, `checks.yml`) — a bare `vitest`/`pnpm test` never touches it, since the default `vitest.config.ts` excludes `test/integration/**` both physically (the directory) and logically (the config). Before adding a new integration test, exhaust the non-spawning alternative first — most "needs a real process" cases turn out not to (see `ExecV3/duration.spec.ts`'s pipe-overlap test: proved with a fixed injected clock and manually-resolved promises, no real timing or spawn at all).
...
`.claude/scripts/*.sh` provision the credentials and permissions the escalation tools (gh, az, Azure DevOps) run on — Keychain items, Entra service principals and certificates, Azure RBAC role assignments, Azure DevOps security groups. Run these by hand, never autonomously by Claude: each is dry-run by default (prints the exact plan, touches nothing) and only takes effect with an explicit `--apply` flag. Read the dry-run output before ever passing `--apply` — these scripts create or change real credentials and permissions on live tenants/subscriptions/orgs.
